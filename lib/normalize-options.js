...
/**
 * @typedef {Object} FSFacadeMethods
 ...
 * @property {FSFacadeMethods} async
 * @property {FSFacadeMethods} sync
  */
 ...
 *
 * @param {FSFacade} [options.facade]
 * Synchronous or asynchronous facades for underlying Node.js File System APIs
 *
...
function normalizeOptions (options, internalOptions) {
  ...
  var fs = Object.assign({}, internalOptions.facade.fs);
  if (options.facade) {
    if (options.facade.async && internalOptions.facade.async) {
      fs = Object.assign({}, internalOptions.facade.fs, options.facade.async);
    }
    if (options.facade.sync && internalOptions.facade.sync) {
      fs = Object.assign({}, internalOptions.facade.fs, options.facade.sync);
    }
  }
