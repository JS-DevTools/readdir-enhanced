...
function normalizeOptions(options, internalOptions) {
  ...
  if (filter != null) {
    if (!filter ||
    (typeof filter !== 'function' && typeof filter !== 'string' && !(filter instanceof RegExp))) {
      throw new TypeError('options.filter must be a function, regular expression, or glob string');
    }

    if (typeof filter === 'string') {
      filter = globToRegExp(filter, {extended: true});
    }
  }
