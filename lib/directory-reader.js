...
let normalizeOptions = require('./normalize-options');
...
DirectoryReader.prototype.readNextDirectory = function readNextDirectory () {
  let reader = this;
  let facade = reader.options.facade;
  let dir = reader.queue.shift();
  reader.pending++;
  ...
  call.safe(facade.fs.readdir, dir.path, (err, items) => {
    ...
    try {
      ...
      reader.emit('error', err2);
      reader.finishedReadingDirectory();
    }
  });
};
