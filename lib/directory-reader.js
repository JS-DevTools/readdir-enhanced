...
function DirectoryReader(facade, dir, options, map) {
  ...
  reader.facade = facade;
  reader.options = normalizeOptions(options);
  reader.map = map;
  ...
  reader.queue = [{ path: dir, basePath: reader.options.basePath, depth: 0 }];
  ...
  reader.stream = new facade.stream.Readable({objectMode: true});
...
/**
 * immediately emits the next piece of data to the reader's stream.
