...
DirectoryReader.prototype.processItem = function processItem(dir, item, done) {
  ...
  var needStats =
    ...
    options.filter ||                                   // we need to filter based on the fs.Stats
  ...
  stat(options.facade.fs, fullPath, function(err, stats) {
    ...
    stats.path = itemPath;
    var meetsCriteria = !options.filter || safeCall(reader, options.filter, stats);
 ...
 * @param {...*} args
 */
function safeCall(reader, fn, args) {
  try {
