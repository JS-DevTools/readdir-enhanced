...
DirectoryReader.prototype.processItem = function processItem(dir, item, done) {
  ...
  var needStats =
    ...
    typeof options.filter === 'function' ||             // we need to filter based on the fs.Stats
...
/**
 * Determines whether the given value meets the user-specified filter criteria.
 * If the user didn't specify a filter, then this function will always return true.
 *
 * @param {string|fs.Stats} value
 * @returns {boolean}
 */
DirectoryReader.prototype.filter = function filter(value) {
  var reader = this;
  var userFilter = reader.options.filter;

  if (!userFilter) {
    return true;
  }
  else if (userFilter instanceof RegExp) {
    return userFilter.test(value.path || value);
  }
  else {
    try {
      ...
      safeEmit(reader.stream, 'error', error);
    }
  }
};
